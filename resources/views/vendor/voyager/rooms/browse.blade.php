                ...
                <div class="panel-body">
                    ...
                    <div class="table-responsive">
                        <table id="dataTable" class="table table-hover">
                            ...
                            <tbody>
                                @php
                                if ((request()->query('room') == 'empty') || (request()->query('room') ==
                                'no-fill-up')) {
                                $nmbr = [];
                                foreach ($dataTypeContent as $room) {
                                $people = $room->people;
                                if (($people->count() == 0) && (request()->query('room') == 'empty')) {
                                $nmbr[] = $room->id;
                                }else if (($people->count() < $room->capability) && (request()->query('room') ==
                                    'no-fill-up')) {
                                    $nmbr[] = $room->id;
                                    }
                                    }
                                    $dataTypeContent = $dataTypeContent->whereIn('id', $nmbr);
                                    }

                                    @endphp
                                    @foreach($dataTypeContent->chunk(10) as
                                    $chunkDdata)
                                    @foreach($chunkDdata as $data)
                                    <tr>
                                        @if($showCheckboxColumn)
                                        <td>
                                        ...
                                        <td>
                                            ...
                                            @if ( property_exists($row->details, 'format') &&
                                            !is_null($data->{$row->field})
                                            )
